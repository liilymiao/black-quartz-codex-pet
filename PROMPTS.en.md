# Reusable Black Quartz Prompts

## Full Codex v2 Companion request
...
- move right/left: no literal running, only restrained directional phase offset and recovery;
- acknowledge: one attached seam facet briefly opens and closes;
- jump: identical sprite size in every frame, changing only y position in a low-middle-high-middle-low arc, with no shadow or scaling;
- failed: core dims and prisms tighten before recovery;
- waiting: prisms open slightly like a permission gate;
- processing: reveal a small embedded core, attached internal ring, and connected black permission thorns;
- review: a narrow embedded scan aperture moves through the seam and returns to center.

Black Quartz has no eyes. Her only aiming feature is the small blue diamond aperture inside the seam. Keep the outer double prism's axis, tips, scale, and baseline fixed; move the aperture inside the seam for 000 up, 090 screen-right, 180 down, and 270 screen-left, then interpolate all sixteen clockwise directions in even 22.5-degree steps. Never rotate the whole object to fake gaze.

Complete the 8×11, 192×208-cell, 1536×2288 spriteVersionNumber 2 atlas. Generate all nine standard rows, four cardinal anchors, and both coherent eight-frame direction rows. Run transparent-edge cleanup, v2 validation, three isolated blind direction reviews, and final visual QA before packaging. Repair any jumping scale change, ambiguous cardinal, metallic hollow-frame drift, or animal-like anatomy before delivery.
```
